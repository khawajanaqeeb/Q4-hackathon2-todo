...
def test_backend_models():
    ...
    try:
        ...
        print("  OK User model has UUID id and username fields")
        ...
        print("  OK Task model has UUID id and user_id fields")
...
def test_api_endpoints():
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            print(f"    FAIL Health endpoint failed: {resp.status_code}")
            return False
        ...
        if resp.status_code == 200:
            print("    OK Verify endpoint working")
        else:
        ...
        if resp.status_code == 200:
            print("    OK Get todos endpoint working")
        else:
            print(f"    FAIL Get todos failed: {resp.status_code} - {resp.text}")
            return False
        ...
        if resp.status_code == 200:
            print("    OK Create todo endpoint working")
            todo_data = resp.json()
            todo_id = todo_data.get('id')
        else:
            print(f"    FAIL Create todo failed: {resp.status_code} - {resp.text}")
            return False
        ...
        if resp.status_code in [401, 403, 422]:  # Auth error is expected, not 404
            print(f"    OK Chat completions endpoint exists (status {resp.status_code})")
        else:
            print(f"    FAIL Chat completions unexpected response: {resp.status_code} - {resp.text}")
            return False

        print("  OK All API endpoints are working")
    ...
    try:
        ...
        with Session(engine) as session:
            ...
            if users:
                ...
                print(f"    OK User-task relationship works: {len(user_tasks)} tasks found for user")
            ...
            if conversations:
                ...
                print(f"    OK Conversation-message relationship works: {len(conv_messages)} messages found for conversation")

        print("  OK Foreign key relationships verified")
...
def run_verification_tests():
    ...
    if all_passed:
        ...
        print("OK All requirements have been met:")

...
def run_comprehensive_test():
    ...
    print(f"   - Register: {resp.status_code} {'✓' if resp.status_code == 200 else '✗'}")
    ...
    print(f"   - Auth cookie set: {'✓' if auth_token else '✗'}")

    # Verify
    print("   - Verifying authentication...")
    resp = make_request("POST", "/auth/verify")
    print(f"   - Verify: {resp.status_code} {'✓' if resp.status_code == 200 else '✗'}")
    ...
    print(f"   - Create Todo: {resp.status_code} {'✓' if resp.status_code == 200 else '✗'}")
    ...
    print(f"   - Get Todos: {resp.status_code} {'✓' if resp.status_code == 200 else '✗'}")
    ...
    if todo_id:
        ...
        print(f"   - Update Todo: {resp.status_code} {'✓' if resp.status_code == 200 else '✗'}")
    ...
    if todo_id:
        ...
        print(f"   - Toggle Todo: {resp.status_code} {'✓' if resp.status_code == 200 else '✗'}")
    ...
    print(f"   - Chat Completions: {resp.status_code} {'✓' if resp.status_code in [200, 400, 401, 403] else '✗'}")
    ...
    if resp.status_code == 200:
        ...
        if user_id:
            ...
            print(f"   - User Conversations: {resp.status_code} {'✓' if resp.status_code in [200, 400, 401, 403] else '✗'}")
            ...
            print(f"   - User Message: {resp.status_code} {'✓' if resp.status_code in [200, 400, 401, 403] else '✗'}")
    ...
    resp = make_request("GET", "/api/todos/")
    print(f"   - /api/todos/: {resp.status_code} {'✓' if resp.status_code == 200 else '✗'}")
    ...
    print(f"   - /chat/completions: {resp.status_code} {'✓' if resp.status_code != 404 else '✗'}")

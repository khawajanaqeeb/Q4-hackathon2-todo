...
def test_chat_endpoints():
    ...
    if chat_resp.status_code == 401:
        ...
        print(f"   ⚠️  Unexpected response: {chat_resp.status_code}")
    ...
    if chat_resp_with_auth.status_code == 401:
        print("   ❌ Still rejecting with valid auth - auth system broken")
        return False
    else:
        print("   ✅ Authenticated request processed (expected various responses)")
    ...
    if verify_resp.status_code == 200:
        print("   ✅ Verify endpoint working with auth")
    else:
        print(f"   ❌ Verify endpoint failed: {verify_resp.status_code}")
        return False

    print("\n✅ All chat endpoint authentication tests passed!")
    return True

if __name__ == "__main__":
    success = test_chat_endpoints()
    if success:
        print("\n🎉 Chat endpoints authentication test completed successfully!")
    else:
        print("\n❌ Chat endpoints authentication test failed!")
        exit(1)
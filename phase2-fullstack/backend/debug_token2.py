...
if decoded:
    ...
    # Let's try decoding without type check
    try:
        raw_decoded = jwt.decode(token, settings.SECRET_KEY, algorithms=[settings.ALGORITHM])
        print(f"Raw decoded: {raw_decoded}")
        print(f"Raw token type: {raw_decoded.get('type')}")
    except Exception as e:
        print(f"Raw decode also failed: {e}")